...
iterations = 1
opencl_ctx = None
opencl_ctx = opencl.pbkdf2_opencl(0, salt, iterations, 0)
...
class Block:
    ...
    def hash_block(self):
        ...
        hash = str(self.timestamp) + str(self.transactions) + str(self.previous_hash) + str(self.nonce)
        opencl_ctx.compile('sha256')
        result = opencl_ctx.run([hash])[0]
        print('result', result)
